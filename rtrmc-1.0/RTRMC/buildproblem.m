...
%
% LAMBDA:     A small smoothing factor, typically set to 1e-6.
...
% SEE ALSO: rtrmc initialguess
    ...
    if ~(strcmpi(class(I), 'uint32') && strcmpi(class(J), 'uint32'))
        I = uint32(I);
        J = uint32(J);
        warning('RTRMC:uint32', 'The index vectors I and J were converted to uint32.');
    end
...
%     % Ir and Jc mimic the way sparse matrices are stored in memory by
%     % Matlab. See http://www.mathworks.com/help/techdoc/apiref/mxsetjc.html
%     % for details.
%     problem.Ir = problem.I - 1;
%     problem.Jc = uint32([ 0 ; cumsum(histc(problem.J, 1:problem.n))]);
    ...
    assert(length(J) == length(I));
    assert(length(X) == length(I));
    assert(length(C) == length(I));
    assert(all(I >= 1) && all(I <= m));
    assert(all(J >= 1) && all(J <= n));
    assert(r <= min(m, n));

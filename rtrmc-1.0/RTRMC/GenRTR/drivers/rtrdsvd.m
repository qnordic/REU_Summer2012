...
% [U,S,V,stats] = rtrdsvd(A,k) returns in addition some statistics from the solver. See RTR for info.
%
% rtrdsvd(A,k,params) allows the user to specify parameters that are passed to the RTR solver.
...
% Universite catholique de Louvain, Departement d'ingenierie mathematique
   ...
   if m == 0 || n == 0,
      error('First argument ''A'' to RTRDSVD must not be empty.');
   end
   if k < 0,
      error('Second argument ''k'' to RTRDSVD must be positive.');
   end
   ...
   if nargout <= 1,
      ...
      if nargout >= 3,
         % RTR DSVD minimizes; computes (-U,V)
